"""
搜索代理子进程管理 — 由 Dashboard 主程序启动/停止 Tavily 兼容搜索代理服务。

搜索代理（tavily_proxy）作为独立子进程运行，监听局域网地址（默认 8002），
供 Cherry Studio 等客户端通过「API 地址 + API 密钥」直接对接 Key 池。
结构与 mcp_manager 一致：Popen + 端口探测 + 安全停止（只清理自己拉起的进程）。
"""
...
from settings import get_settings, proxy_url, proxy_urls

_lock = threading.Lock()
_proc: subprocess.Popen | None = None
# 本程序实际启动过的代理服务进程 PID 集合（以端口监听为准）。
...
def _python_exe() -> str:
    """代理依赖 tavily 库：打包后用 exe 自身；开发模式优先使用 .venv 解释器。"""
...
def _entry_args() -> list[str]:
    """构造启动搜索代理子进程的命令行。"""
    if getattr(sys, "frozen", False):
        # 打包后单 exe：Tavily.exe --proxy 进入搜索代理角色
        return [sys.executable, "--proxy"]
    # 开发模式：python dashboard.py --proxy（与 exe 共用同一角色入口）
    return [_python_exe(), str(Path(__file__).resolve().parent / "dashboard.py"), "--proxy"]
...
# 前端每 5 秒轮询 /api/proxy/status，netstat 是重量级子进程调用；
...
def _find_pid_on_port(port: int, force: bool = False) -> int | None:
    ...
    try:
        ...
        # 轮询 /api/proxy/status 都会弹出一个命令行窗口又马上关闭。
        out = subprocess.run(
...
def start() -> dict:
    ...
    with _lock:
        ...
        if is_port_open("127.0.0.1", port):
            return {"ok": False, "error": f"端口 {port} 已被占用，可能已有搜索代理在运行"}
        ...
        try:
            ...
            return {"ok": False, "error": f"搜索代理启动后立即退出: {reason}"}
