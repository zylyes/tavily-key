...
页面提供「API Key 列表」「统计」「请求日志」「MCP 服务」「搜索代理」「Research 任务」「文档」「设置」八个标签页：API Key 列表展示 Key 状态、用量与异常徽章，用量列含**额度耗尽预测**（按近 7 天日均消耗估算「约 N 天后耗尽」，7 天内标黄）；**统计页**含关键指标卡（总请求 / 近 24h 成功失败与成功率 / 总积分 / 剩余积分 / 活跃 Key）、用量趋势图（近 7/30 天按天聚合请求数与积分）、接口分布（近 24h 各接口成功/失败横向条形）与积分消耗按天柱状图；请求日志支持按接口 / Key / 成功失败 / 时间范围筛选、分页、**CSV 导出**与 **清理**；「MCP 服务」页可查看/启停 MCP 服务并配置其启动方式与网络参数；「搜索代理」页查看/启停 Tavily 兼容 REST 代理并配置密钥；「Research 任务」页展示 `wait=false` 提交的异步研究任务状态看板（request_id / 归属 Key / 状态 / 结果摘要，带缓存与查询上限），失败/错误任务支持**一键重试**（复用原提交参数）；「文档」页内置 wiki（左侧目录 + 右侧 Markdown 渲染，离线可读）；「设置」页可配置部署模式、绑定域名、监听地址、端口、访问令牌（`auth_token`）以及开机自启、托盘行为等通用设置。设置了访问令牌后，所有 `/api/*` 请求需携带 `X-Auth-Token` 请求头，未通过鉴权时页面会弹出登录层。
