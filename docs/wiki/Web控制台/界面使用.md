...
| 关于与更新 · 立即更新 | `POST /api/update/download`、`GET /api/update/status`、`POST /api/update/pause`、`POST /api/update/resume`、`POST /api/update/cancel`、`POST /api/update/apply` | `updater.start_download()/get_download_status()/pause_download()/resume_download()/cancel_download()/apply_update()`（仅打包版） |
