...
<cite>本页内容依据以下源文件整理：前端页面 [file://app/dashboard.html](file://app/dashboard.html) 与后端路由 [file://app/dashboard.py](file://app/dashboard.py)。</cite>
...
Dashboard 是一个单页应用：`app/dashboard.html` 负责全部 DOM 结构与前端交互，`app/dashboard.py` 提供 FastAPI 后端路由。页面自上而下为头部、信息栏、标签页菜单与内容区，通过四个标签页切换面板：
...
对应的核心 HTML 骨架如下（完整结构见 [file://app/dashboard.html](file://app/dashboard.html)）：
...
整个页面只有一条数据链路：前端 `load()` → `GET /api/stats` → 渲染信息栏与各面板。触发时机有四类：

1. **页面加载时**执行一次；
2. **每 10 秒**自动轮询（`setInterval(load, 10000)`）；
3. **每次操作后**（添加、删除、停用/激活、健康检查、更新用量）立即执行；
4. **MCP 服务状态**每 5 秒轮询一次（`setInterval(loadMcp, 5000)`），页面加载时还会拉取 MCP 设置并尝试自动复制已运行服务的地址到剪贴板。
