...
| `usage_pct` | 积分使用百分比，由 `ApiKey.usage_pct` 计算（`credits_used / credits_limit * 100`，保留 1 位小数） |
...
| 请求抛出异常（超时、鉴权失败等） | `DEAD tvly-xxx****xxxx [错误信息]` | 自动停用该 Key |
...
> ⚠️ 注意：`check_health()` 会对**任何异常**（包括临时网络超时、限流）执行停用操作。若某 Key 因瞬时故障被停用，需要人工确认后重新激活。另外，探测请求本身会计入 Tavily 的调用配额（`max_results=1` 的轻量搜索）。
