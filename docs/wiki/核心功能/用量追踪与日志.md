...
- **官方对账层**：通过 Tavily 官方 `/usage` 同步 billing cycle 真实用量（CLI `usage` 命令、面板「同步用量」按钮，或 v0.10.0 起的**后台定时自动同步** `usage_auto_sync_hours`，默认 6 小时），供仪表盘展示、月度重置自动恢复与异常识别（泄露/耗尽）对账。
...
| `high_error_rate` | 近 24h 服务器错误率 > `error_rate`（忽略客户端请求错误，如 HTTP 400/参数校验） |
...
页面由新前端渲染（Vue 3 + Vite 构建产物 `web/dist/index.html`；开发时为项目根 `web/dist`，打包后从资源目录 `_MEIPASS/web/dist` 读取），数据通过以下接口获取：
...
- **官方用量同步**：CLI `usage --sync`、面板「同步用量」按钮，或后台按 `usage_auto_sync_hours`（默认 6h，0=关闭）周期自动同步触发，从 Tavily 官方 `/usage` 拉取 billing cycle 真实用量并回写（含月度重置自动恢复——exhausted Key 在官方用量归零后自动恢复，不再依赖手动同步）；不依赖本地统计，避免本地漏记导致面板数据偏低。
