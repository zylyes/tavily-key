...
| `next_key_least_used()` | 选择请求数最少的 Key |
...
- **负载均衡**：`next_key()` 先按 `last_used_at` 升序取活跃 Key，再用 `_next_index` 做轮询游标；`next_key_least_used()` 则按 `request_count` 升序取第一个。
...
| 方法与路径 | 功能 | 数据来源 |
| --- | --- | --- |
| `GET /` | 返回 Dashboard 页面 | 内存中的 `DASHBOARD_HTML` |
...
| `GET/POST /api/settings` | 读取 / 保存部署设置（config.json） | `settings` 模块 |
...
`if __name__ == "__main__"` 分支从 `config.json` 读取 host/port（由 `settings` 模块提供）并通过 `uvicorn.run(app, host=..., port=...)` 启动服务。
...
1. `_get_client()` 按 `KEY_STRATEGY` 从池中取出一个 Key，构造 `TavilyClient`；
...
其中 `tavily_pool_status` 是唯一的运维类工具，不消耗外部 API 配额，用于让 AI 客户端了解当前 Key 池的健康与用量情况。
...
- **切换负载均衡策略**：修改 `mcp_server.py` 顶部的 `KEY_STRATEGY` 为 `"least-used"` 或 `"round-robin"`。
- **数据库并发**：多进程同时读写依赖 WAL 模式与 busy_timeout，不要随意改动这两条 PRAGMA；若部署多副本，需确保 `DB_PATH` 指向共享存储。
- **仪表盘模板**：`dashboard.py` 启动时会读取 `templates/dashboard.html` 文件，该文件缺失会导致启动失败。