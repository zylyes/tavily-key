...
[dashboard.py](file://app/dashboard.py) 基于 FastAPI，在模块加载时读取 `app/dashboard.html` 作为页面（打包后从资源目录读取），通过 `/api/*` 端点与浏览器交互。它同样只是 KeyPool 的薄封装——每个 API 端点直接调用对应 KeyPool 方法：
