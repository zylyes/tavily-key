...
数据库文件默认位于 `key_pool.py` 同目录下的 `tavily_keys.db`，包含两张表：
...
[dashboard.py](file://app/dashboard.py) 基于 FastAPI，在模块加载时读取 `templates/dashboard.html` 作为页面，通过 `/api/*` 端点与浏览器交互。它同样只是 KeyPool 的薄封装——每个 API 端点直接调用对应 KeyPool 方法：
...
| /api/settings | GET/POST | 读取 / 保存 config.json 部署设置 |

## 接入端三 MCP Server

[mcp_server.py](file://app/mcp_server.py) 是把 KeyPool 能力接入 LLM 生态的关键模块。它通过 `FastMCP` 注册 6 个工具：`tavily_search`、`tavily_extract`、`tavily_crawl`、`tavily_map`、`tavily_research`（业务工具）与 `tavily_pool_status`（池状态查询）。

所有业务工具遵循统一的三段式流程：

1. **取 Key**：`_get_client()` 依据 `KEY_STRATEGY` 策略从 KeyPool 选取一个活跃 Key，构造 `TavilyClient`；
...
`KEY_STRATEGY` 是模块级常量，当前默认 `round-robin`，可切换为 `least-used`。选 Key 逻辑：

```mermaid
flowchart TD
    R["MCP 工具收到请求"] --> S{"KEY_STRATEGY"}
    S -->|"round-robin（默认）"| RR["next_key()<br/>活跃 Key 按 last_used_at 升序<br/>游标 _next_index 轮转取下一个"]
    S -->|"least-used"| LU["next_key_least_used()<br/>按 request_count 升序取第一条"]
    RR --> OK["返回 (raw_key, masked)"]
    LU --> OK
    OK --> C["TavilyClient(raw_key)"]
    C --> A["Tavily API"]
```

成功时从响应中的 `usage.credits` 读取实际积分消耗；若响应未携带 usage 则按搜索深度估算（basic=1、advanced=2）。
...
sequenceDiagram
    ...
    End->>KP: next_key() / next_key_least_used()
...
- **可替换策略**：`next_key_least_used()` 提供按累计请求数升序的变体，MCP Server 用 `KEY_STRATEGY` 常量一行切换。
