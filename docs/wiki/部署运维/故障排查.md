...
> 面板会同时展示三种地址：局域网 IP、**主机名地址**（`http://<主机名>.local:8001/sse`，mDNS，切换网络不变；Windows 设备也可用裸主机名 `http://<主机名>:8001/sse`）、本机地址（`http://127.0.0.1:8001/sse`，仅本机）。切换网络后 IP 会变化，建议客户端配置使用主机名地址则无需改动。
