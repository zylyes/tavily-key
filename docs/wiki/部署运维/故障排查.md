...
| `database disk image is malformed` | SQLite 文件损坏 | 删除 `tavily_keys.db` 后重启，自动重建 |
...
数据库文件路径固定在项目根目录（[file://app/key_pool.py](file://app/key_pool.py)）：
...
```bash
rm -f tavily_keys.db tavily_keys.db-wal tavily_keys.db-shm
./scripts/run_dashboard.sh
```
...
>
>   ```bash
>   cp tavily_keys.db tavily_keys.db.bak
>   ```
>
