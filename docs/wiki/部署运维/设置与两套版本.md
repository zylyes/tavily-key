...
项目所有部署相关的设置统一存放在 `data/config.json`（由 `settings.py` 模块读写，首次启动自动生成默认值）：
...
Dashboard 界面顶部新增「**设置**」标签页，提供与 `data/config.json` 等价的图形化配置：
...
> 注意：`/`（HTML 页面）本身不鉴权，但页面内数据接口均受保护。设置令牌前请牢记令牌，忘记时可直接编辑 `data/config.json` 重置。
...
```mermaid
graph TD
    A[同一代码库] --> B[data/config.json mode]
